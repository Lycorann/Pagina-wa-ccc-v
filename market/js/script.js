...
function selectColor(e){
    ...
    // eliminar activo de todos los botones
...
function addToCartHandler(e) {
    ...
    const colorOptions = button.parentElement.querySelector('.color-options')?.querySelectorAll('.color-option');
    let color = selectedColor;
    
    if(!color && colorOptions && colorOptions.length > 0){
        const activeColor = Array.from(colorOptions).find(opt => opt.classList.contains('active'));
        color = activeColor ? activeColor.getAttribute('data-color') : colorOptions[0].getAttribute('data-color');
        if (!activeColor) colorOptions[0].classList.add('active');
    }
...
function addToCart(id, name, price, color){
    ...
    // guardar carrito y actualizar la interfaz del usuario
    saveCart();
    updateCartUI();
...
// Actualizar la interfaz del usuario del carrito
...
function checkout(){
    ...
    const total = cart.reduce((sum, item) => sum + (item.price * item.quantity), 0);
    const confirmed = confirm(`¿Estás seguro de que quieres finalizar tu compra por un total de $${total.toFixed(2)}?`);
    
    if (confirmed){
        // En una aplicación real, esto redirigiría a una página de pago
        // o procesaría el pago. Para esta demostración, solo mostraremos un mensaje.
...
function showNotification(message){
    ...
    // Animar salida y eliminar después de 3 segundos
    setTimeout(() => {
